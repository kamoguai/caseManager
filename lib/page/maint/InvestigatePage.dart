...
///工程會勘案件處理list頁面
///Date: 2021-01-29
class InvestigatePage extends StatefulWidget {
  final String accName;
  final String deptId;
  InvestigatePage({this.accName, this.deptId});
  @override
  _InvestigatePageState createState() => _InvestigatePageState();
}

class _InvestigatePageState extends State<InvestigatePage>
    with
        AutomaticKeepAliveClientMixin<InvestigatePage>,
        MyListState<InvestigatePage> {
  ///app bar左邊title
  var userTitle = "單位案件";
  ...
  void _callApiDataExt(Map<String, dynamic> map) async {
    ...
    var searchStatus =
        map['SearchStatus'] == null ? '' : '${map['SearchStatus']}';
    var searchCaseType =
        map['SearchCaseType'] == null ? '' : map['SearchCaseType'];
    var searchSubject =
        map['SearchSubject'] == null ? '' : map['SearchSubject'];
    ...
    var res = await DPMaintDao.getInvestigateList(
        iType: 1,
        userId: userInfo.userData.UserID,
        searchFunit: deptId,
        ...
        searchCustNo: searchCustNO,
        searchPuser: searchPUser);
    if (res != null && res.result) {
      List<MaintTableCell> list = new List();
      dataArray.addAll(res.data);
      if (dataArray.length > 0) {
  ...
  getFirstTimeApiData(String deptId) async {
    var res = await DPMaintDao.getInvestigateList(
        iType: null, userId: userInfo.userData.UserID, searchFunit: deptId);
    if (res != null && res.result) {
      List<MaintTableCell> list = new List();
      dataArray.addAll(res.data);
      if (dataArray.length > 0) {
          ...
          child: Column(
            ...
            children: <Widget>[
              Card(
                  child: SearchDialog(
                userId: userInfo.userData.UserID,
                deptId: userInfo.userData.DeptID,
                isDPCase: true,
                callApiDataExt: _callApiDataExt,
              )),
            ],
          ),
        )));
