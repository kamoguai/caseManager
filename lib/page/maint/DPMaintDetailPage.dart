...
///單位案件詳情頁面
///Date: 2019-06-27
class DPMaintDetailPage extends StatefulWidget {
  ///由前頁傳入客編
  final custCode;
  ///由前頁傳入使用者id
  final userId;
...
class _DPMaintDetailPageState extends State<DPMaintDetailPage> with BaseWidget{
  ...
  getCaseData() async {
    isLoading = true;
    var res = await DPMaintDao.getDPMaintDetail(userId: widget.userId, caseId: widget.caseId);
  ...
  Widget detailReportDialog(BuildContext context) {
    return Material(
      ...
      child: Container(
        ...
        child: DetailReportDialog(deptName: userInfo.userData.DeptName, takeName: userInfo.userData.UserName, userId: userInfo.userData.UserID, caseId: widget.caseId, statusName: model.statusName, caseTypeName: model.caseTypeName, fromFunc: 'DPMaint', userInfo: userInfo, accName: widget.accName, deptId: widget.deptId,)
      )
    );
  }
  ///訊號dialog
