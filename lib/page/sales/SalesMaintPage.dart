...
///裝機問題通知列表頁面
///Date: 2019-07-01
class SalesMaintPage extends StatefulWidget {
  
  final String accName;
  SalesMaintPage({this.accName});
  
  @override
  _SalesMaintPageState createState() => _SalesMaintPageState();
}

class _SalesMaintPageState extends State<SalesMaintPage> with AutomaticKeepAliveClientMixin<SalesMaintPage>, MyListState<SalesMaintPage>{
  ///app bar左邊title
  var userTitle = "業務單位";
  ///部門id
  var deptId = "";
  ...
  getApiData() async {
    ...
    var res = await SalesMaintDao.getSalesMaintList(userId: userInfo.userData.UserID, deptId: userInfo.userData.DeptID, searchStatus: searchStatusValue);
    return res;
  }
  ///查詢actions
  List<Widget> searchActions() {
    var statusType = ["新案","接案","結案","部門結案"];
    List<Widget> wList = [];
    for (var dic in statusType) {
      wList.add(
  ...
  ///查詢dialog
  showSelectorStatusSheetController(BuildContext context) {
      ...
      child: Row(
        ...
        children: <Widget>[
          ...
          GestureDetector(
            ...
            onTap: (){
              showSelectorStatusSheetController(context);
            },
          ),
          Container(
            alignment: Alignment.center,
