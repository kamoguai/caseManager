...
class DPMaintDao {
  ...
  static postDPMaintClose({userId, caseId}) async {
    var res = await HttpManager.netFetch(
        Address.didDeptClose(userId, caseId), null, null, null);
    if (res != null && res.result) {
      if (res.data['Response']['ReturnCode'] == "00") {
  ...
  static getDPMaintDetail({userId, caseId}) async {
    ...
    var res = await HttpManager.netFetch(
        Address.getDeptCaseDetail(userId, caseId), null, null, null);
    if (res != null && res.result) {
      if (Config.DEBUG) {
  ...
  ///單位回覆作業
  static didDPMaint(
      {userId, caseId, newStatus, newAData, deptId, pUserId}) async {
    var res = await HttpManager.netFetch(
        Address.assignDPMaint(
            userId, caseId, deptId, pUserId, newStatus, newAData),
        null,
        null,
        null);
    if (res != null && res.result) {
  ...
  static getDPMaintCloseList({userId, deptId}) async {
    ...
    List<dynamic> dataArray = [];
    var res = await HttpManager.netFetch(
        Address.getDeptCloseList(userId, deptId), null, null, null);
    if (res != null && res.result) {
      if (Config.DEBUG) {
  ...
  static getDPMaintCloseDetail({userId, caseId}) async {
    ...
    Map<String, dynamic> dataArray = {};
    var res = await HttpManager.netFetch(
        Address.getDeptCloseCase(userId, caseId), null, null, null);
    if (res != null && res.result) {
      if (Config.DEBUG) {
  ...
  static getInvestigateList(
    ...
    if (res != null && res.result) {
      if (Config.DEBUG) {
        print("工程會勘案件list resp => " + res.data.toString());
      }
      if (res.data['Response']['ReturnCode'] == "00") {
        mainDataArray = res.data["ReturnData"];
      }
